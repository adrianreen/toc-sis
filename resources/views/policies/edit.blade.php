        ...
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="mb-6">
                    ...
                    <ol class="inline-flex items-center space-x-1 md:space-x-3">
                        ...
                        <li>
                            <div class="flex items-center">
                                ...
                                <a href="{{ route('policies.show', $policy) }}" class="ml-1 text-sm font-medium text-gray-500 hover:text-gray-700 md:ml-2 cursor-pointer">{{ Str::limit($policy->title, 20) }}</a>
                            </div>
                        </li>
                        <li aria-current="page">
                            <div class="flex items-center">
                                ...
                                <span class="ml-1 text-sm font-medium text-gray-500 md:ml-2">Edit Policy</span>
                            </div>
                        </li>
                    </ol>
                </nav>
                ...
                <div class="mt-4">
                    ...
                    <p class="mt-2 text-gray-600">Update policy details and content</p>
                </div>
            </div>
            ...
            <x-card>
                <div class="p-8">
                    <form action="{{ route('policies.update', $policy) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
                        @csrf
                        @method('PUT')
                            ...
                            <div class="grid grid-cols-1 gap-6">
                                ...
                                <div>
                                    ...
                                    <textarea name="description" 
                                              ...
                                              placeholder="Brief description of what this policy covers...">{{ old('description', $policy->description) }}</textarea>
                                    @error('description')
                                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                    @enderror
                                ...
                                <div>
                                    ...
                                    <select name="policy_category_id" 
                                        ...
                                        @foreach($categories as $category)
                                            <option value="{{ $category->id }}" {{ old('policy_category_id', $policy->policy_category_id) == $category->id ? 'selected' : '' }}>
                                                {{ $category->name }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div>
                                    ...
                                    <select name="scope" 
                                        ...
                                        <option value="college" {{ old('scope', $policy->scope) === 'college' ? 'selected' : '' }}>College-wide</option>
                                        <option value="programme" {{ old('scope', $policy->scope) === 'programme' ? 'selected' : '' }}>Programme-specific</option>
                                    </select>
                                    @error('scope')
                                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                    @enderror
                                ...
                                <div>
                                    ...
                                    <select name="programme_type" 
                                        ...
                                        <option value="all" {{ old('programme_type', $policy->programme_type) === 'all' ? 'selected' : '' }}>All Programmes</option>
                                        <option value="elc" {{ old('programme_type', $policy->programme_type) === 'elc' ? 'selected' : '' }}>ELC Programmes</option>
                                        <option value="degree_obu" {{ old('programme_type', $policy->programme_type) === 'degree_obu' ? 'selected' : '' }}>Degree (OBU) Programmes</option>
                                        <option value="qqi" {{ old('programme_type', $policy->programme_type) === 'qqi' ? 'selected' : '' }}>QQI Programmes</option>
                                    </select>
                        ...
                        <div class="border-t border-gray-200 pt-8">
                            ...
                            @if($policy->hasFile())
                                <div class="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
                                    <h3 class="text-sm font-medium text-blue-900 mb-2">Current File</h3>
                                    <div class="flex items-center justify-between">
                                        <div class="flex items-center">
                                            <svg class="w-8 h-8 text-red-500 mr-3" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                                <path d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z"/>
                                            </svg>
                                            <div>
                                                <p class="text-sm font-medium text-blue-900">{{ $policy->file_name }}</p>
                                                <p class="text-xs text-blue-700">{{ $policy->getFileSizeHuman() }} • Uploaded {{ $policy->updated_at->format('M j, Y') }}</p>
                                            </div>
                                        </div>
                                        <a href="{{ route('policies.download', $policy) }}" 
                                           class="inline-flex items-center px-3 py-1 bg-blue-600 text-white text-xs rounded-lg hover:bg-blue-700 transition-colors cursor-pointer">
                            ...
                            <div class="mb-6">
                                <label for="policy_file" class="block text-sm font-medium text-gray-700 mb-2">
                                    {{ $policy->hasFile() ? 'Replace Policy Document (PDF)' : 'Policy Document (PDF)' }}
                                </label>
                                <div class="mt-1">
                                    ...
                                    <div id="upload-zone" class="relative border-2 border-dashed border-gray-300 rounded-xl p-8 text-center hover:border-blue-400 transition-all duration-300 bg-gradient-to-br from-gray-50 to-white">
                                        ...
                                        <div id="upload-prompt" class="pointer-events-none">
                                            ...
                                            <h3 class="text-lg font-semibold text-gray-900 mb-2">{{ $policy->hasFile() ? 'Replace Policy Document' : 'Upload Policy Document' }}</h3>
                                            ...
                                            <p class="text-xs text-gray-500">PDF files up to 10MB {{ $policy->hasFile() ? '(leave blank to keep current file)' : '' }}</p>
                                        </div>
                                        ...
                                        <div id="file-preview" class="hidden pointer-events-none">
                                            ...
                                            <h3 class="text-lg font-semibold text-gray-900 mb-2">New File Ready to Upload</h3>
                            ...
                            <div>
                                ...
                                <textarea name="content" 
                                          ...
                                          placeholder="Optional: Add text content that will be displayed on the policy page...">{{ old('content', $policy->content) }}</textarea>
                                @error('content')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div>
                                    ...
                                    <input type="datetime-local" 
                                           name="published_at" 
                                           id="published_at"
                                           value="{{ old('published_at', $policy->published_at?->format('Y-m-d\TH:i')) }}"
            ...
            if (form && statusSelect) {
                form.addEventListener('submit', function(e) {
                    ...
                    if (actionButton && actionButton.name === 'action') {
                        ...
                        // For 'update' action, keep the current status selection
                    }
                });
            }
